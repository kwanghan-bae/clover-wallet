...
## 2024-05-20 - Destructive Actions and hitSlop
**Learning:** Using `hitSlop` to increase the touch target of an icon button (like a Trash icon for deletion) is great for accessibility but can lead to accidental data loss if the action is destructive and executes immediately without confirmation.
**Action:** Always wrap destructive actions triggered by buttons with `hitSlop` in an `Alert.alert` confirmation dialog to prevent accidental triggers while maintaining a large, accessible tap area.
