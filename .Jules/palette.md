...
## 2024-07-13 - Destructive Action Confirmation in Lists
**Learning:** In React Native, destructive actions (like delete buttons) in list items should always be wrapped in an `Alert.alert` confirmation dialog. This is especially critical when `hitSlop` is used to increase the tap area, as users might accidentally trigger the action while scrolling or tapping nearby elements. This prevents accidental data loss and provides a safer UX.
**Action:** When implementing destructive actions in list items or highly interactive areas, always use `Alert.alert` to prompt for confirmation before executing the action.
