...
## 2024-05-24 - Destructive Actions in List Items
**Learning:** List items often contain destructive actions (like delete buttons) that use `hitSlop` to increase their tappable area for mobile usability. This increased touch area, while good for accessibility, raises the risk of accidental taps causing unintended data loss if the action is immediate.
**Action:** Always wrap immediate destructive actions (like deleting a history record) in an `Alert.alert` confirmation dialog. This provides a safety net against accidental taps, especially important on touch screens where lists are scrolled frequently.
