...
## 2026-05-21 - Accessible Complex Cards
**Learning:** When complex card components (like PostCard or SpotListItem containing authors, stats, and badges) are wrapped in a TouchableOpacity with a simplistic accessibilityLabel (e.g., 'View Post'), screen readers completely skip the rich internal data. Users miss critical context like likes, comments, or win counts.
**Action:** For interactive cards, synthesize a comprehensive accessibilityLabel on the parent pressable that summarizes all critical child data (e.g., author, title, key stats) so screen reader users get the full context before navigating.
