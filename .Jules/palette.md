...
## 2024-06-13 - Destructive Action Confirmation in List Items
**Learning:** Destructive actions (like delete buttons) inside list items (e.g., `HistoryItem`) are prone to accidental taps, especially when their touch target is enlarged using `hitSlop`.
**Action:** Always wrap destructive actions in an `Alert.alert` confirmation dialog to provide a safety net against accidental data loss.
