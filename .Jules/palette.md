...
## 2024-03-22 - Destructive Action Confirmation
**Learning:** Destructive actions in list items (like delete buttons) that use `hitSlop` or padding to increase touch targets are prone to accidental taps by users scrolling through lists. This can lead to unintended data loss.
**Action:** Always wrap destructive action callbacks within list items in an `Alert.alert` confirmation dialog, providing explicit 'Cancel' and 'Delete' (with `style: 'destructive'`) options to safeguard user data.
