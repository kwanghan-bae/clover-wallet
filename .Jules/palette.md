...
## 2024-08-14 - Destructive Actions with Large Touch Targets
**Learning:** Using `hitSlop` or padding to increase the touch target size of destructive buttons (like delete icons in a list) significantly increases the risk of accidental taps, leading to unintended data loss.
**Action:** When a button triggers a destructive action and has an enlarged touch target, always wrap the action in an `Alert.alert` confirmation dialog to prevent accidental data loss.
