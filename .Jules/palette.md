...
## 2024-05-22 - HitSlop and Destructive Actions
**Learning:** In React Native, when using `hitSlop` to artificially increase the tap target size of destructive action buttons (like a delete icon in a list item), it significantly increases the risk of accidental taps and unwanted data loss.
**Action:** Always wrap destructive actions in an `Alert.alert` confirmation dialog, especially when the button's touch target is expanded using `hitSlop` or padding classes, to ensure users explicitly confirm the action before it executes.
