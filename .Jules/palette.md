...
## 2024-08-02 - Destructive Action Confirmation in List Items
**Learning:** Destructive actions (like deletion) in list items where the touch target is artificially expanded (e.g., using `hitSlop` in `HistoryItem`) are prone to accidental triggering, especially during scroll interactions. Without an explicit confirmation step, users can accidentally lose data.
**Action:** Always wrap destructive actions in list items with an `Alert.alert` confirmation dialog (or similar pattern) to require explicit user intent before proceeding, particularly when `hitSlop` is heavily relied upon to increase accessibility targets.
