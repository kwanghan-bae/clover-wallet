...
## 2024-05-19 - Destructive Actions in List Items
**Learning:** In React Native, destructive actions (like delete buttons) placed within list items are often wrapped in `TouchableOpacity` or `Pressable` with increased touch areas using `hitSlop` to improve accessibility. While this makes them easier to tap, it significantly increases the risk of accidental taps leading to unintended data loss.
**Action:** Always wrap destructive actions in list items in an `Alert.alert` confirmation dialog to explicitly ask the user for confirmation before executing the destructive function, especially when `hitSlop` is used.
