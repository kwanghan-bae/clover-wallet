...
## 2024-08-13 - Confirm Destructive Actions on Small Touch Targets
**Learning:** In list items where destructive actions (like 'Delete' icons) use `hitSlop` to increase their tappable area on mobile devices, the risk of accidental taps and subsequent data loss significantly increases, especially when these icons are placed near other interactive elements.
**Action:** Always wrap destructive actions, particularly icon-only buttons using `hitSlop` within list items (like `HistoryItem`), in an `Alert.alert` confirmation dialog to explicitly require user intent before proceeding with the action.
