...
## 2026-05-18 - Confirmation Dialogs for Destructive Actions with Enlarged Touch Targets
**Learning:** Destructive actions (like delete buttons in lists) often use `hitSlop` to increase their tappable area for better mobile accessibility. However, this enlarged area increases the risk of accidental taps, leading to unintended data loss and a frustrating user experience.
**Action:** Always wrap destructive actions in an `Alert.alert` confirmation dialog before execution, especially when `hitSlop` or generous padding is used to increase the touch target.
