...
## 2024-05-18 - Destructive Action Confirmations
**Learning:** Destructive actions (like delete buttons) in list items without a confirmation step can easily lead to accidental data loss, especially on mobile devices where `hitSlop` is used to increase the touch target size, making errant taps more likely.
**Action:** Always wrap destructive UI actions (e.g., "delete history item") in an `Alert.alert` confirmation dialog before executing the underlying action.
