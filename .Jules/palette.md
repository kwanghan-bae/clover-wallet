...
## 2024-05-19 - Destructive Action Confirmations
**Learning:** Destructive actions (like deleting items) in list items without a confirmation dialogue cause an anxiety-inducing experience, especially since mobile `hitSlop` makes accidental taps highly probable. In list items like `HistoryItem`, users reported accidental deletion of records.
**Action:** Always wrap destructive actions in an `Alert.alert` confirmation dialog to prevent accidental data loss. This adds a critical friction point before permanently deleting user data. In test files, remember to mock `Alert.alert` (e.g., `jest.spyOn(Alert, 'alert')`) and manually execute the `onPress` callback in the provided button arrays.
