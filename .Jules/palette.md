...
## 2024-10-31 - Destructive Actions and hitSlop Vulnerability
**Learning:** In React Native lists, using `hitSlop` to increase the touch target size of icon buttons is great for accessibility, but it introduces a critical UX vulnerability for destructive actions (like delete). Users can accidentally tap these expanded areas while scrolling, leading to unintended data loss.
**Action:** Destructive icon buttons inside list items (or any component where `hitSlop` might overlap with common touch areas like scrolling) must always be wrapped in an `Alert.alert` confirmation dialog before executing the action.
