...
## 2024-05-19 - Destructive Action Confirmation in List Items
**Learning:** In React Native list items (like `HistoryItem`), using `hitSlop` to increase touch targets for small icons (like a trash bin) makes accidental taps much more likely. If the action is destructive (like deleting a record) and happens instantly without confirmation, it can lead to frustrating data loss for users.
**Action:** Always wrap destructive actions (e.g., delete buttons) in list items with an `Alert.alert` confirmation dialog to prevent accidental triggers, ensuring the dialog clearly states the consequences and requires explicit user confirmation.
