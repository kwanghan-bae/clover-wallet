...
## 2026-08-06 - Confirmation Dialog for Destructive Actions
**Learning:** In React Native, destructive actions like deleting an item from a list (e.g., `HistoryItem`) can be accidentally triggered, especially when touch targets are enlarged for accessibility using `hitSlop`. Without confirmation, this can lead to frustrating data loss for users.
**Action:** Always wrap destructive list item actions (like 'Delete') in an `Alert.alert` confirmation dialog before executing the action. This ensures safety against accidental taps, especially on mobile where touch targets overlap or are expanded.
