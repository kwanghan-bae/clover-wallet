...
## 2024-10-24 - Destructive Action Confirmation in Lists
**Learning:** Destructive actions (like delete buttons) in list items often have their tap areas expanded using `hitSlop` to improve accessibility for touch interfaces. However, this increases the risk of accidental data loss when users scroll or interact with nearby elements.
**Action:** Always wrap destructive actions in list items with an `Alert.alert` confirmation dialog before proceeding with the action, ensuring interaction safety for users who may accidentally tap the expanded touch target.
