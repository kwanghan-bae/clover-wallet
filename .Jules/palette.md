...
## 2024-07-26 - Destructive Actions and Alert Confirmation
**Learning:** In React Native, destructive actions (like delete buttons) in list items should always be wrapped in an `Alert.alert` confirmation dialog to prevent accidental data loss. This is especially crucial when using `hitSlop` which intentionally increases the tap area of buttons to improve accessibility, but consequently increases the risk of unintended taps.
**Action:** Always wrap destructive function calls (e.g., `onDelete`) within an `Alert.alert` array configuration with explicit `cancel` and `destructive` button styles.
