...
## 2026-06-10 - Confirmation Dialogs for Destructive List Actions
**Learning:** In React Native, destructive actions (like delete buttons) in list items should always be wrapped in an `Alert.alert` confirmation dialog. This is especially critical because `hitSlop` is often used to increase the tap area for accessibility, which inadvertently increases the likelihood of accidental taps and unwanted data loss.
**Action:** When implementing delete or other destructive actions within list components (like `HistoryItem`), always prompt the user with a confirmation dialog before executing the action.
