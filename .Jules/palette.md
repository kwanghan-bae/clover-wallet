...
## 2024-06-25 - Confirmation for Destructive Actions in Lists
**Learning:** In React Native, list items often use `hitSlop` to increase the touch target for small icons (like a delete button). This increases the risk of accidental taps, leading to unintended data loss if actions are executed immediately.
**Action:** Always wrap destructive actions (like delete buttons) in list items within an `Alert.alert` confirmation dialog to provide a safety net against accidental taps.
