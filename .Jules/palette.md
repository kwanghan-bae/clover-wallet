...
## 2024-10-24 - Lotto History Deletion Safety
**Learning:** In the lotto history `FlatList` (`HistoryScreen`), the `HistoryItem` delete button uses a large `hitSlop` (10px) to meet touch target accessibility. While scrolling through numerous lotto records, users are prone to accidentally tapping this enlarged target, leading to unintended record deletion.
**Action:** When increasing touch targets (`hitSlop` or padding) for destructive actions within scrollable lists, always implement an explicit confirmation step (like `Alert.alert`) to balance physical accessibility with data safety.
