...
## 2024-07-25 - Destructive Action Confirmation
**Learning:** In React Native, destructive actions (like delete buttons) in list items often lack confirmation dialogs, which can lead to accidental data loss. This is especially problematic when `hitSlop` is used to increase the tap area, making accidental taps more likely on small mobile screens.
**Action:** Always wrap destructive actions in an `Alert.alert` confirmation dialog with a clear warning and 'destructive' styled confirm button to prevent accidental user errors and improve overall safety in the UX.
