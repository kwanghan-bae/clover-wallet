...
## 2024-09-11 - Destructive Actions in List Items
**Learning:** In React Native, list items with destructive actions (like a delete icon) often use `hitSlop` to increase the tap area. This makes accidental presses more likely, leading to unintentional data loss without a confirmation step.
**Action:** Always wrap destructive actions in list items within an `Alert.alert` confirmation dialog to confirm user intent and prevent accidental deletions.
