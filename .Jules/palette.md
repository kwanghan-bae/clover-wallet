...
## 2026-05-16 - Destructive Action Confirmation and hitSlop
**Learning:** In React Native list items, increasing touch target areas using `hitSlop` is beneficial for accessibility but introduces a high risk of accidental data loss when applied to destructive actions (like a delete button).
**Action:** Destructive actions inside list items, particularly those with expanded touch areas via `hitSlop`, should always be wrapped in an `Alert.alert` confirmation dialog to prevent unintentional deletions.
