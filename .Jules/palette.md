...
## 2024-05-20 - Destructive Action Confirmation in List Items
**Learning:** In React Native, list items often contain destructive action buttons (like a delete icon) that utilize `hitSlop` to increase their touch target for better accessibility. However, this increased tappable area also increases the risk of accidental presses, leading to unintended data loss if the action is immediate.
**Action:** Always wrap destructive actions within list items (especially those with enhanced touch targets) in an `Alert.alert` confirmation dialog to prevent accidental data loss and improve overall UX safety.
