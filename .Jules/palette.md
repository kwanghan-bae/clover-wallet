...
## 2026-08-05 - Prevent Accidental Deletions on Increased Touch Targets\n**Learning:** In React Native, destructive actions (like delete buttons) in list items should always be wrapped in an `Alert.alert` confirmation dialog. This is especially critical because `hitSlop` is often used to increase the tap area for accessibility, which simultaneously increases the risk of users accidentally tapping the destructive action when trying to interact with the list item itself.\n**Action:** When implementing or reviewing `TouchableOpacity` or `Pressable` components for destructive actions that utilize `hitSlop`, always ensure a confirmation step (e.g., `Alert.alert`) is present to mitigate accidental data loss.
