...
## 2023-10-27 - React Native Accessibility Label Override Insight
**Learning:** In React Native, applying an `accessibilityLabel` to a touchable container (like `TouchableOpacity` or `Pressable`) completely overrides the accessibility text of any child components. If the container includes dynamic visual data alongside an icon (e.g., a like button with a like count), that data will be entirely hidden from screen readers unless it is explicitly included in the parent's `accessibilityLabel`.
**Action:** When creating interactable UI elements that group an icon and text (such as stats, counts, or dynamic statuses), ensure the parent container's `accessibilityLabel` combines both the action's intent and the current data value (e.g., `accessibilityLabel={isLiked ? \`좋아요 취소, 좋아요 \${likes}개\` : \`좋아요, 좋아요 \${likes}개\`}`).
