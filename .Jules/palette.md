...
## 2024-10-25 - Confirmation Dialog for Destructive Actions
**Learning:** Destructive actions (like deleting items) in list items can easily lead to accidental data loss if triggered immediately upon touch. This is especially true when `hitSlop` is applied to increase the tap area for mobile accessibility, making accidental taps more likely.
**Action:** Always wrap destructive actions in React Native with an `Alert.alert` confirmation dialog to verify user intent before executing the destructive function.
