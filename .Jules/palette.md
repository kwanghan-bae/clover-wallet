...
## 2026-05-24 - Confirmation Dialog for Destructive Actions
**Learning:** In React Native lists where items contain destructive actions (like delete) with an enlarged tap area via `hitSlop`, users are highly susceptible to accidental data loss from stray touches.
**Action:** Always wrap destructive list item actions (e.g., delete buttons) in an `Alert.alert` confirmation dialog prior to executing the operation, and verify the behavior by spying on `Alert.alert` in Jest tests.
