...
## 2024-05-29 - Destructive Actions Confirmation
**Learning:** In React Native, destructive actions (like delete buttons) in list items can easily be triggered accidentally by users, especially when `hitSlop` is used to increase the touch area for accessibility. Without a confirmation, this leads to accidental data loss and a frustrating user experience.
**Action:** Always wrap destructive actions in list items or forms with an `Alert.alert` confirmation dialog to explicitly confirm the user's intent before executing the action.
