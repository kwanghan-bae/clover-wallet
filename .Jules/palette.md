...
## 2024-07-25 - Confirmation Dialogs for Destructive Actions
**Learning:** Destructive actions (like delete buttons) in list items should always be wrapped in a confirmation dialog (`Alert.alert`) on mobile. This is especially critical when `hitSlop` is used to enlarge touch targets, as it significantly increases the risk of accidental data loss from misplaced taps.
**Action:** When implementing or updating destructive actions in React Native (e.g., deleting a history item or post), always add a confirmation `Alert.alert` before executing the actual deletion callback.
