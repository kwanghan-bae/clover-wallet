...
## 2026-06-19 - Destructive Actions in List Items
**Learning:** In React Native, inline delete buttons within list items (like `HistoryItem`) can easily lead to accidental data loss due to miss-taps, especially when `hitSlop` is used to enlarge touch targets. Immediate deletion without confirmation causes high user frustration.
**Action:** Destructive actions embedded in fast-paced list views must always be intercepted with an `Alert.alert` (or equivalent native confirmation dialog) to verify user intent before executing the deletion callback.
