...
## 2024-06-25 - Confirmation Dialogs for Destructive Actions in Lists
**Learning:** Destructive actions (like delete buttons) embedded in list items often require enlarged touch targets (e.g., via `hitSlop`) for basic usability on mobile. However, this increased tappable area also increases the risk of accidental mis-taps, leading to frustrating data loss since list items are frequently scrolled.
**Action:** Always wrap destructive actions in list items with an `Alert.alert` confirmation dialog before executing the deletion to prevent accidental data loss.
