...
*   **날짜**: 2025-12-24 (2026-04 갱신)
*   **결정**: NestJS(TypeScript) 백엔드와 React Native(Expo) 프론트엔드의 모노레포 구조 채택. (초기에는 Kotlin/Spring Boot였으나 NestJS로 마이그레이션 완료)
*   **이유**: 단일 저장소에서 백엔드와 프론트엔드의 데이터 규격을 즉시 동기화하고, 통합 배포 파이프라인(Render)을 관리하기 위함. TypeScript 통일로 프론트/백 간 타입 공유 가능.
...
*   **날짜**: 2026-04-11
*   **상태**: 계획됨
*   **결정**: 기존 수동 모노레포 관리를 Turborepo로 전환하여 빌드/테스트 파이프라인을 통합 관리.
*   **이유**: backend-node와 frontend 간의 빌드 캐싱, 병렬 실행, 의존성 그래프 기반 태스크 오케스트레이션이 필요해짐. Turborepo는 설정이 간단하고 npm 워크스페이스와 자연스럽게 통합됨.
*   **영향**: 루트 `package.json`에 워크스페이스 설정 추가, `turbo.json`으로 파이프라인 정의, CI에서 `turbo run` 사용.
...
*   **날짜**: 2026-04-11
*   **상태**: 계획됨
*   **결정**: backend-node와 frontend에서 공통으로 사용하는 타입, 상수, 유틸리티를 `packages/shared` 패키지로 분리.
*   **이유**: 동일한 API 응답 타입, 에러 코드, 상수를 양쪽에서 중복 정의하고 있어 불일치 위험이 존재함. 단일 출처(Single Source of Truth)로 타입 안전성을 확보하기 위함.
*   **영향**: `packages/shared/` 디렉토리 생성, TypeScript 경로 별칭 설정, 양쪽 프로젝트에서 `@clover/shared` import 사용.

---

## [ADR-014] 150줄 파일 크기 제한 규칙
*   **날짜**: 2026-04-11
*   **결정**: 모든 소스 파일(*.ts, *.tsx)은 최대 150줄로 제한하며, 초과 시 모듈 분리를 의무화.
*   **이유**: 파일이 커질수록 인지 부하가 증가하고 코드 리뷰 품질이 저하됨. AI 에이전트 협업 시에도 작은 파일이 컨텍스트 효율이 높음. 150줄은 단일 책임 원칙(SRP)을 자연스럽게 강제하는 실용적 임계값.
*   **영향**: ESLint 커스텀 룰 또는 pre-commit 가드에서 검증. 기존 초과 파일은 점진적으로 분리.

---

## [ADR-015] 80% 테스트 커버리지 CI 강제
*   **날짜**: 2026-04-11
*   **결정**: CI 파이프라인에서 테스트 커버리지 80% 미만 시 빌드를 실패 처리.
*   **이유**: 테스트가 충분하지 않은 코드의 배포를 방지하고, TDD 문화를 정착시키기 위함. 80%는 비즈니스 로직의 핵심 경로를 커버하면서도 과도한 테스트 부담을 주지 않는 균형점.
*   **영향**: Jest 설정에 `coverageThreshold` 추가, GitHub Actions에서 커버리지 리포트 생성 및 검증.