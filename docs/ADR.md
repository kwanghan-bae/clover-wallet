...
## [ADR-006] 런타임 데이터 검증 및 에러 바운더리 도입
*   **날짜**: 2025-12-24
*   **결정**: `zod`를 통한 API 응답 스키마 검증과 `GlobalErrorBoundary`를 통한 앱 전체 크래시 방지 로직 도입.
*   **이유**: 비동기 데이터 로딩 중 발생하는 예측 불가능한 런타임 에러로부터 사용자 경험을 보호하고 디버깅 효율을 높이기 위함.