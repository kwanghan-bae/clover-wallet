...
describe('BadgeService', () => {
  ...
  describe('updateUserBadges', () => {
    ...
    it('should add FIRST_WIN badge if user has any winning', async () => {
      (prisma.user.findUnique as jest.Mock).mockResolvedValue({ id: userId, badges: '' });
      ...
      expect(prisma.user.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: { badges: expect.stringContaining(BadgeService.BADGE_FIRST_WIN) },
        }),
      );
    });
    ...
    it('should add LUCKY_1ST badge if user won 1st place', async () => {
      ...
      (prisma.lottoGame.findFirst as jest.Mock).mockResolvedValue({ id: BigInt(100) });

      await service.updateUserBadges(userId);

      expect(prisma.user.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: { badges: expect.stringContaining(BadgeService.BADGE_LUCKY_1ST) },
        }),
      );
    });
    ...
    it('should add FREQUENT_PLAYER badge if user played 10+ games', async () => {
      ...
      expect(prisma.user.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: { badges: expect.stringContaining(BadgeService.BADGE_FREQUENT_PLAYER) },
        }),
      );
    });
    ...
    it('should add extraction badges based on win method', async () => {
      (prisma.user.findUnique as jest.Mock).mockResolvedValue({ id: userId, badges: '' });
      (prisma.lottoGame.count as jest.Mock).mockResolvedValue(0);
      (prisma.lottoGame.findFirst as jest.Mock).mockImplementation(({ where }) => {
        if (where.extractionMethod === 'DREAM') return Promise.resolve({ id: BigInt(1) });
      ...
      expect(prisma.user.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: { badges: expect.stringContaining(BadgeService.BADGE_DREAM_MASTER) },
        }),
      );
    });
  });
