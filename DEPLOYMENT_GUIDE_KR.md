...
3.  이름(예: `clover-wallet-db`), 비밀번호(**꼭 기억해두세요!**), 지역(예: Seoul 또는 Tokyo)을 입력합니다.
...
4.  GitHub 저장소(`clover-wallet`)를 연결합니다.
5.  **서비스 설정:**
...
## 3. 검증
1.  빌드가 완료될 때까지 기다립니다.
2.  Render가 제공하는 URL을 확인합니다 (예: `https://clover-wallet-api.onrender.com`).
3.  Health check 또는 API 엔드포인트를 테스트합니다:
    `https://clover-wallet-api.onrender.com/actuator/health` (활성화된 경우) 또는 알려진 API 엔드포인트.
