    ...
    <SafeAreaView className="flex-1 bg-[#F5F7FA] dark:bg-dark-bg">
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <GlassCard className="p-6">
            <View className="flex-row items-start justify-between">
              <View className="flex-1">
                <Text style={{ fontFamily: 'NotoSansKR_700Bold' }} className="text-2xl text-[#1A1A1A] dark:text-dark-text mb-2">
                  {spotInfo?.name || "정보 없음"}
                </Text>
                <View className="flex-row items-center mb-4">
                  <MapPin size={16} color="#4CAF50" />
                  <Text style={{ fontFamily: 'NotoSansKR_400Regular' }} className="text-[#757575] dark:text-dark-text-secondary text-sm ml-1 flex-1">
            ...
            <View className="flex-row justify-around border-t border-gray-100 dark:border-dark-card pt-5 mt-2">
              ...
              <View className="w-[1] h-10 bg-gray-100 dark:bg-dark-card" />
              <View className="items-center">
        ...
        <View className="px-5 mt-8">
          <Text style={{ fontFamily: 'NotoSansKR_700Bold' }} className="text-lg text-[#1A1A1A] dark:text-dark-text mb-4">당첨 내역</Text>
          ...
          {history && history.length > 0 ? (
            ...
            <View className="bg-white dark:bg-dark-surface rounded-2xl p-10 items-center justify-center border border-gray-100 dark:border-dark-card">
        ...
        {travelPlans && travelPlans.length > 0 && (
          <View className="px-5 mt-8">
            <Text style={{ fontFamily: 'NotoSansKR_700Bold' }} className="text-lg text-[#1A1A1A] dark:text-dark-text mb-4">🗺️ 주변 여행 코스</Text>
            {travelPlans.map((plan: TravelPlan) => (
              <View
                key={plan.id}
                className="bg-white dark:bg-dark-surface rounded-2xl p-5 mb-3 border border-gray-100 dark:border-dark-card"
                style={{ shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 6, elevation: 2 }}
              >
                <View className="flex-row items-center justify-between mb-2">
                  <Text style={{ fontFamily: 'NotoSansKR_700Bold' }} className="text-[#1A1A1A] dark:text-dark-text text-base flex-1 mr-2" numberOfLines={1}>
                    {plan.title}
                  </Text>
                ...
                <Text style={{ fontFamily: 'NotoSansKR_400Regular' }} className="text-[#757575] dark:text-dark-text-secondary text-sm mb-3" numberOfLines={2}>
                  {plan.description}
                </Text>
