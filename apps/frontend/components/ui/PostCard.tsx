...
const PostCardComponent = ({
  ...
  const isOwner =
    currentUser != null && currentUser.id === post.userSummary?.id;
  ...
  return (
    <TouchableOpacity
      ...
      accessibilityLabel={`${nickname} 작성. ${post.title ? post.title + ", " : ""}${post.content.substring(0, 30)}. 좋아요 ${post.likes}개, 댓글 ${commentCount}개. 두 번 탭하여 게시글 보기`}
    >
      {/* Header */}
      <View className="flex-row items-start mb-4">
        ...
        >
          <View className="w-10 h-10 rounded-full bg-[#4CAF50]/10 items-center justify-center">
            <AppText variant="title" className="text-primary-text">
              {initial}
            </AppText>
          </View>
        </TouchableOpacity>
        <View className="flex-1">
          <TouchableOpacity
            ...
            accessibilityLabel={`${nickname} 프로필 보기`}
          >
            <AppText variant="title" className="text-text-primary text-[15px]">
              {nickname}
            </AppText>
          </TouchableOpacity>
          <AppText
            variant="body"
            className="text-text-muted text-[12px] mt-0.5"
          >
            {dateStr}
          </AppText>
        </View>
      ...
      <AppText
        variant="body"
        className="text-text-primary text-[15px] leading-[22.5px] mb-4"
        numberOfLines={4}
      >
        {post.content}
      </AppText>
      ...
      <View className="flex-row items-center px-1">
        ...
        >
          ...
          <AppText
            variant="body"
            className={`ml-1.5 ${post.isLiked ? "text-[#EF5350]" : "text-text-muted"}`}
          >
            {post.likes}
          </AppText>
        </TouchableOpacity>
        ...
        >
          ...
          <AppText variant="body" className="ml-1.5 text-text-muted">
            {commentCount}
          </AppText>
        </View>
        <View className="flex-1" />
