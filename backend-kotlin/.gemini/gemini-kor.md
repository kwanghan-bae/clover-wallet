...
-   **글로벌 공통 지침:** 최상위 폴더의 `.gemini/gemini.md` 또는 `.gemini/gemini-kor.md` 파일에 정의된 공통 원칙을 항상 준수해야 합니다.
...
2.  **문서 참조**: 위에 명시된 절대 경로의 `Clover_Lotto_README.md`와 `WORK_LOG_backend.md` 파일을 읽어 전체 맥락과 현재 Task를 파악합니다.
3.  **구현**: 백엔드 코드를 작성하거나 수정합니다.
4.  **작업 로그 업데이트**: 구현 완료 후, 즉시 `WORK_LOG_backend.md` 파일에 작업 내역과 Task 변경 사항을 업데이트합니다.
